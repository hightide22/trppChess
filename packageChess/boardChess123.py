import piecesChess123

class Board:
    height = 8
    width = 8

    space = type(piecesChess123.EmptySpace(0, 0))

    board = [["-"] * width] * height

    def isStepOkay(self, x, y):
        if x >= Board.height or y >= Board.width or x < 0 or y < 0: return False
        if type(Board.board[y][x]) == Board.emptySpace:
            return True
        return False

    def isKillOkay(self, x, y, color):
        if x >= Board.height or y >= Board.width or x < 0 or y < 0: return False

    def setStandartBoard(self):
        if Board.width != 8 or Board.height != 8: return
